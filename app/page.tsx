...
const defaultAppearance: Appearance = { theme: "purple", font: "modern", fontSize: "standard" };
...
export default function Home() {
  ...
  return <div className="shell">
    <aside className="sidebar">
      ...
      <nav><div className="nav-title">工作区</div><button className="nav-item active"><Icon name="grid"/>项目总览</button><div className="nav-title row"><span>我的项目</span><span>{projects.length}</span></div><div className="project-list">{projects.map(item => <button key={item.id} className={`project-link ${selectedId === item.id ? "selected" : ""}`} onClick={() => setSelectedId(item.id)}><span className="dot" style={{ background: item.color }}/><span className="ellipsis">{item.name}</span><Icon name="arrow"/></button>)}</div></nav>
    ...
    <main>
      <header className="topbar"><div><p className="eyebrow">今天，专注关键进展</p><h1>{project?.name || "欢迎回来"}</h1></div><div className="top-actions"><button className="secondary reminder-button" onClick={enableReminders}><Icon name="calendar"/>开启提醒</button>{project && <button className="secondary assistant-trigger" onClick={() => setAssistantOpen(true)}><Icon name="spark"/>智能安排</button>}{project && <button className="icon-button top-delete" title="删除项目" onClick={removeProject}><Icon name="trash"/></button>}<button className="avatar">我</button></div></header>
      {error && <div className="toast error"><span>{error}</span><button onClick={() => setError("")}>×</button></div>}
      {!project ? <EmptyState onCreate={() => setModal("project")}/> : <>
    ...
    {modal === "project" && <ProjectModal onClose={() => setModal(null)} onCreated={async created => { setModal(null); await loadProjects(); setSelectedId(created.id); }} onError={setError}/>} 
...
function PlanningAssistant({ project, open, onClose, onApplied, onError }: { project: ProjectDetail; open: boolean; onClose: () => void; onApplied: () => Promise<void>; onError: (text: string) => void }) {
  ...
  const [input, setInput] = useState(""); const [busy, setBusy] = useState(false); const [autoApply, setAutoApply] = useState(false); const [pendingPlan, setPendingPlan] = useState<AssistantPlan | null>(null); const endRef = useRef<HTMLDivElement>(null);
  useEffect(() => { if (open) endRef.current?.scrollIntoView({ behavior: "smooth" }); }, [messages, open]);
  useEffect(() => { setMessages([{ role: "assistant", content: `已切换到“${project.name}”。告诉我你的时间安排或想推进的目标，我来帮你拆解。` }]); setPendingPlan(null); }, [project.id, project.name]);
  const apply = async (plan: AssistantPlan) => { setBusy(true); try { const result = await request<{ appliedCount: number }>(`/api/projects/${project.id}/assistant`, { method: "POST", body: JSON.stringify({ apply: true, plan }) }); setMessages(current => [...current, { role: "assistant", content: `已将 ${result.appliedCount} 项安排写入任务看板。你可以点击卡片继续修改。` }]); setPendingPlan(null); await onApplied(); } catch (err) { onError(err instanceof Error ? err.message : "应用安排失败"); } finally { setBusy(false); } };
  const send = async () => { const content = input.trim(); if (!content || busy) return; const nextMessages: AssistantMessage[] = [...messages, { role: "user", content }]; setMessages(nextMessages); setInput(""); setBusy(true); setPendingPlan(null); try { const data = await request<{ reply: string; plan: AssistantPlan }>(`/api/projects/${project.id}/assistant`, { method: "POST", body: JSON.stringify({ messages: nextMessages.slice(-10) }) }); setMessages(current => [...current, { role: "assistant", content: data.reply }]); if (data.plan.tasks.length) { if (autoApply) await apply(data.plan); else setPendingPlan(data.plan); } } catch (err) { onError(err instanceof Error ? err.message : "智能助手暂时无法响应"); } finally { setBusy(false); } };
  return <aside className={`assistant-panel ${open ? "open" : ""}`} aria-hidden={!open}><div className="assistant-head"><div><span><Icon name="spark"/></span><div><b>Chrona 智能助手</b><small>规划行程 · 安排时间 · 自动建任务</small></div></div><button onClick={onClose}>×</button></div><div className="assistant-context"><span>正在规划</span><b>{project.name}</b><label><input type="checkbox" checked={autoApply} onChange={event => setAutoApply(event.target.checked)}/>自动写入任务</label></div><div className="chat-messages">{messages.map((message, index) => <div className={`chat-message ${message.role}`} key={`${message.role}-${index}`}><span>{message.role === "assistant" ? "C" : "我"}</span><p>{message.content}</p></div>)}{busy && <div className="chat-message assistant"><span>C</span><p className="typing">正在整理安排<i/><i/><i/></p></div>}{pendingPlan && <div className="plan-card"><div><span>安排建议</span><b>{pendingPlan.tasks.length} 项任务</b></div><ul>{pendingPlan.tasks.slice(0, 5).map(task => <li key={`${task.title}-${task.dueDate}`}><span className={`quadrant-mini ${task.priority}`}/><p>{task.title}<small>{task.dueDate || "待定日期"} · {task.estimateHours}h</small></p></li>)}</ul><button disabled={busy} onClick={() => apply(pendingPlan)}><Icon name="check"/>确认并写入看板</button></div>}<div ref={endRef}/></div><div className="quick-prompts"><button onClick={() => setInput("根据当前任务，帮我安排今天的工作顺序和时间段")}>安排今天</button><button onClick={() => setInput("检查截止日期和工作量，帮我调整未来一周的计划")}>规划本周</button><button onClick={() => setInput("找出当前风险和最容易拖延的任务，给我应对建议")}>检查风险</button></div><div className="chat-input"><textarea rows={2} value={input} onChange={event => setInput(event.target.value)} onKeyDown={event => { if (event.key === "Enter" && !event.shiftKey) { event.preventDefault(); send(); } }} placeholder="例如：我明天下午有 3 小时，优先推进样机测试…"/><button disabled={busy || !input.trim()} onClick={send}><Icon name="send"/></button></div></aside>;
}
